...
export const GET = createApiHandler(
  ...
  async (request: NextRequest, { pool, user }) => {
    const mainList = await pool.query(
      `
    ...
    const allList = await pool.query(
      `
      SELECT DISTINCT place_id AS "placeId"
      FROM (
        SELECT place_id FROM saved_places WHERE user_id = $1
        UNION
        SELECT fi.place_id 
        FROM saved_place_folder_items fi
        JOIN saved_place_folders f ON fi.folder_id = f.id
        WHERE f.user_id = $1
      ) combined
      `,
      [user!.id]
    ...
    return Response.json(
      { 
        placeIds: mainList.rows.map((row: any) => row.placeId),
        allPlaceIds: allList.rows.map((row: any) => row.placeId)
      },
